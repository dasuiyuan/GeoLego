...
import static lite.VectorMapRenderUtils.buildTransform;
...
public class PipelineBuilder {
     ...
     * @param paintArea
     * @param sourceCrs
     * @param overSampleFactor 化简因子 1=no oversampling, 4=four time oversample (generialization will be 1/4 pixel)
     * @param queryBuffer
     * @return
     * @throws FactoryException
     */
    private static Context createContext(
            ReferencedEnvelope tagArea,
            Rectangle paintArea,
            CoordinateReferenceSystem sourceCrs,
            double overSampleFactor,
            int queryBuffer)
        ...
        //地理空间范围
        context.renderingArea = tagArea;
        //屏幕空间范围
        context.paintArea = paintArea;
        //原始坐标系
        context.sourceCrs = sourceCrs;
        //地理到屏幕转换（仿射变换矩阵）
        context.worldToScreen = RendererUtilities.worldToScreenTransform(tagArea, paintArea);
        context.queryBuffer = queryBuffer;

        //可能的情况下使用连续的转换包装
        final boolean wrap = false;
        //源坐标系到目标坐标空间的投影器
        context.projectionHandler = ProjectionHandlerFinder.getHandler(tagArea, sourceCrs, wrap);

//        CoordinateReferenceSystem tagCrs = context.renderingArea.getCoordinateReferenceSystem();
//        context.sourceToTargetCrs = buildTransform(sourceCrs, tagCrs);

        //仿射变换转为线性变换
        context.targetToScreen = ProjectiveTransform.create(context.worldToScreen);
        //源空间参考到平面坐标系的变换
        context.sourceToScreen = context.targetToScreen;
    ...
    /**
     * 几何对象展开（如果Multi对象内只有一个Geometry，则变为Single对象）
     */
    ...
    public static final class ClipRemoveDegenerateGeometries extends Clip {
        ...
        private Geometry collectionClip(GeometryCollection geom) throws Exception {
            ...
            for (int t = 0; t < geom.getNumGeometries(); t++) {
                Geometry g = geom.getGeometryN(t); //todo: JUST-GIS use t to loop sub geometies
